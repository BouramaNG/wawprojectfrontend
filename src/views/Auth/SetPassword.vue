    ...
    <div class="max-w-md w-full space-y-8">
      ...
      <div class="text-center">
        ...
        <p class="mt-1 text-sm text-gray-500">
          Veuillez définir un mot de passe sécurisé pour activer votre compte.
        </p>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg p-8 space-y-6">
        ...
        <div class="text-center">
          <p class="text-xs text-gray-500">
            Ce lien expire dans 24 heures. Si vous avez des questions, contactez-nous à
            <a href="mailto:si@wawtelecom.com" class="text-primary-600 hover:text-primary-700">si@wawtelecom.com</a>
          </p>
        </div>
      </div>
    </div>
...
const validateToken = async () => {
  ...
  try {
    const response = await api.get('/partner-auth/validate-invitation', {
...
const handleSubmit = async () => {
  ...
  try {
    const response = await api.post('/partner-auth/set-password', {
      token: token.value,
    ...
    if (response.data.success) {
      ...
      router.push({
        name: 'login',
        query: { message: 'Mot de passe défini avec succès. Vous pouvez maintenant vous connecter.' },
      })
    } else {
...
onMounted(async () => {
  ...
  if (!token.value) {
    error.value = 'Token d\'invitation manquant dans l\'URL'
    return
  }
